...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleNewRequest = (message: any) => {
      if (message.type === 'NEW_REQUEST' && message.record) {
        if (message.record.origin === origin) {
          setRecords(prev => [message.record, ...prev]);
        }
      }
    };

    chrome.runtime.onMessage.addListener(handleNewRequest);
    
    // Auto-refresh interval (backup)
    const interval = setInterval(init, 5000);
    return () => {
      clearInterval(interval);
      chrome.runtime.onMessage.removeListener(handleNewRequest);
    };
  }, [origin]);
      ...
      {activeTab === 'meta' && (
        <MetaTab 
          seo={{
            ...
            description: "DevSphere analysis for " + origin,
            ogTitle: "DevSphere Protected Origin",
            ogImage: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=200&h=200"
          }}
        />
      )}
